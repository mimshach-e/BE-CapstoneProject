...
# Testing the Product App Functionalities

class ProductsTestCase(TestCase):
    # Setting up variables and users
    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_user(
            username='testuser', email='testuser@alx.com', password='testing@123')
        self.admin_user = User.objects.create_superuser(
            username='admin', email='admin@alx.com', password='admin@123')
    ...
    def test_create_category(self):
        ...
        self.assertEqual(Category.objects.get(
            name='New Category').created_by, self.admin_user)
    ...
    def test_create_product(self):
        ...
        self.assertEqual(Product.objects.get(
            name='New Product').created_by, self.user)
    ...
    def test_list_products(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        # Considering pagination set to 10 per page
    ...
    def test_create_rating(self):
        self.client.force_authenticate(user=self.user)
        url = reverse('product-ratings-list',
    ...
    def test_create_wishlist(self):
        self.client.force_authenticate(user=self.user)
        url = reverse('product-wishlist-list',
                      kwargs={'product_id': self.product.id})
        data = {
    ...
    def test_create_discount(self):
        self.client.force_authenticate(user=self.admin_user)
        url = reverse('product-discount-list',
                      kwargs={'product_id': self.product.id})
        data = {
            'name': 'Xmas Bonus',
    ...
    def test_product_filter(self):
        ...
        response = self.client.get(
            url, {'category': self.category.id, 'price_gte': 5, 'price_lte': 15})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data['results']), 1)
